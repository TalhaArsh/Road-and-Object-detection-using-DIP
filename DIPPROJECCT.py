...
def detectLanes(frame, cropMarginPercent):
    ...
    cv2.imshow("combined", yellowMask)
...
def identifyObstacles(frame):
    ...
    cv2.imshow('Obstacle Detection Mask', obstacleMask)
...
def main(videoPath):
    ...
    prevLeft = None
    prevRight = None
    prevTime, frameCount, fps = time.time(), 0, 0
    while cap.isOpened():
        ret, frame = cap.read()
        if not ret:
            break
...
if __name__ == "__main__":
    videoPath = "./DIP Project Videos (1)/vid1.mp4"
    main(videoPath)